...
class StatusEvent(TypedDict):
    """Status message (S) received from the Host Command service."""
...
class EnhancedLogEvent(TypedDict):
    """Enhanced log (EL) message received from the Host Command service."""

    tag: Literal[EventType.ENHANCED_LOG]
    log: str
...
class EventStream(CommandClient):
    def __init__(
        self,
        host: str,
    ...
    def subscribe(
        ...
        if isinstance(event_filter, EventType):
            subscription = (callback, lambda event: event["tag"] == event_filter)
        elif isinstance(event_filter, Iterable):
            subscription = (callback, lambda event: event["tag"] in event_filter)  # type: ignore[operator]
        else:
    ...
    async def subscribe_status(
        ...
        remove_subscription = self.subscribe(
            callback,
            lambda event: event["tag"] == EventType.STATUS
    ...
    async def subscribe_enhanced_log(
        ...
        if self._enhanced_log_subscribers[log_type] == 0:
            await self.send("ELAGG 1 ON")
            await self.send(f"ELENABLE 1 {log_type} ON")
            await self.send(f"ELLOG {log_type} ON")
        ...
        async def unsubscribe() -> None:
            with suppress(ClientConnectionError):
                self._enhanced_log_subscribers[log_type] -= 1
                if self._enhanced_log_subscribers[log_type] == 0:
                    await self.send(f"ELLOG {log_type} OFF")
                remove_subscription()
    ...
    async def _message_handler(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                while True:
                    message = await conn.readuntil(self._delimiter)
                    message = message.rstrip()
                    self._logger.debug("Received message: %s", message)
                    self._parse_message(message)

            except ClientConnectionError:
                # Pass through to retry logic below
                pass
    ...
    def _parse_message(self, message: str) -> None:
        ...
        if message.startswith("S:"):
            ...
            self.emit({"tag": EventType.ENHANCED_LOG, "log": message[4:]})
    ...
    async def _resubscribe(self) -> None:
        ...
        for log_type, count in self._enhanced_log_subscribers.items():
            if count > 0:
                await self.send("ELAGG 1 ON")
                await self.send(f"ELENABLE {log_type} ON")
                await self.send(f"ELLOG {log_type} ON")
