...
def on_object_updated(event: ObjectUpdated[Button]) -> None:
    """Print out any state changes."""
    print(f"[Button updated] '{event.obj.name}' ({event.obj.id})")
...
async def main() -> None:
    ...
    async with Vantage(
        ...
        vantage.buttons.subscribe(ObjectUpdated, on_object_updated)
