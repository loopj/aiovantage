...
async def main() -> None:
    ...
    async with ConfigClient(args.host, args.username, args.password) as client:
        ...
        print("# Vantage Areas")
        async for area in get_objects(client, object_type="Area"):
            print(area)
        print()

        # Dump all Loads using the get_objects helper
        print("# Vantage Loads")
        async for load in get_objects(client, object_type="Load"):
            print(load)
        print()

        # Dump some StationObjects using the get_objects helper
        print("# Vantage Stations")
        async for station in get_objects(client, object_type=("Keypad", "EqCtrl")):
            print(station)
        print()
