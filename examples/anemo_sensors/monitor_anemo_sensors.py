...
def on_object_updated(event: ObjectUpdated[AnemoSensor]) -> None:
    """Print out any state changes."""
    print(f"[AnemoSensor updated] '{event.obj.name}' ({event.obj.id})")
...
async def main() -> None:
    ...
    async with Vantage(
        ...
        vantage.anemo_sensors.subscribe(ObjectUpdated, on_object_updated)
