...
class ACIClient:
    ...
    async def initialize(self):
        ...
        # Connect
        self._reader, self._writer = await asyncio.open_connection(self._host, self._port, ssl=self._ssl_context, limit=1024*1024*10)
    ...
    async def request(self, interface, method, params=None):
        """Build and send an RPC request."""

        def _params_to_xml(data, parent):
    ...
    async def fetch_objects(self, object_types=None, per_page=50):
        ...
        # Create a filter
        response = await self.request("IConfiguration", "OpenFilter", {
            "Objects": None,
            "XPath": xpath,
        })
        ...
        results = []
        while True:
            response = await self.request("IConfiguration", "GetFilterResults", {
                "Count": per_page,
                "WholeObject": True,
                "hFilter": handle,
            })
    ...
    async def _login(self):
        # Send login command
        response = await self.request("ILogin", "Login", {
            "User": self._username,
